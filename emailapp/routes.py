...
def home():
    ...
    current_month_obj = None
    for y in current_user.years:
        if y.num == current_year:
            current_year_obj = y
            for m in y.months:
                if m.num == current_month_int:
                    current_month_obj = m
                    break

    # get all of the months training intensities
    diff_intensities = []
    diff_inten_names = []
    if current_month_obj != None:
        for i in current_month_obj.training_intensities:
            if i.name not in diff_inten_names:
                diff_inten_names.append(i.name)
                diff_intensities.append(i)
    # get list of traning categories in that month
    tcategories = []
    if current_month_obj != None:
        for c in current_month_obj.training_categories:
            if c not in tcategories:
                tcategories.append(c) """
    ...
    # FOR DELETE
    """     curr_categories = current_user.categories
    user_trainings = current_user.trainings
    curr_week = datetime.datetime.now().strftime("%W")
    # check if there are unused weeks
    for week in current_user.weeks:
        if week.trainings == []:
            db.session.delete(week)
            db.session.commit()
    existing_weeks = current_user.weeks
    existing_weeks.sort(key=lambda x: x.weeknum) """
...
""" 
    # DELETE
    curr_categories=curr_categories, 
...
""" 
    # checking if the user has logged in and assigning stuff for it
    ...
    if request.method == 'POST':
        ...
        # creating a new_training object with name and user
        new_training = Training(name=name, user=current_user)
        # adding following properties to new_training instance
        body = request.form['body']
        if body != '':
            new_training.body = body
        huolto = request.form['huolto']
        if huolto != '':
            new_training.huolto = int(huolto)
        pk = request.form['pk']
        if pk != '':
            new_training.pk = int(pk)
        vk = request.form['vk']
        if vk != '':
            new_training.vk = int(vk)
        mk = request.form['mk']
        if mk != '':
            new_training.mk = int(mk)
        hypyt = request.form['hypyt']
        if hypyt != '':
            new_training.hypyt = int(hypyt)
        raw_week_input = request.form['weekinput']
        if raw_week_input.startswith('0'):
            weekinput = int(raw_week_input[1])

            # create Week class to new_training
        week_assigned = False
        weeklist = []
        for week in existing_weeks:
            weeklist.append(week.weeknum)

            if week.weeknum == weekinput:
                new_training.week = week
                week_assigned = True
                break
        if not week_assigned:
            if weekinput == curr_week:
                new_week = Week(user=current_user)
                try:
                    db.session.add(new_week)
                    db.session.commit()
                except:
                    return 'error adding the new_week to db'

                new_training.week = new_week
            else:
                notcurr_week = Week(user=current_user, weeknum=weekinput)
                try:
                    db.session.add(notcurr_week)
                    db.session.commit()
                except:
                    return 'error adding the notcurr_week to db'

                new_training.week = notcurr_week

        # assigning the new training to new or an existing category
        categoryname = request.form['category']
        category_assigned = False
        for item in curr_categories:
            if item.name == categoryname:
                new_training.category = item
                category_assigned = True
                break
        # if existing categories weren't found create a new one and assign it to
        # our new_training
        if not category_assigned:
            new_category = Category(name=categoryname, user=current_user)
            try:
                db.session.add(new_category)
                db.session.commit()
            except:
                return 'there was an error creating a new category'

            new_training.category = new_category
        # add the new_training to the database
        try:
            db.session.add(new_training)
            db.session.commit()
        except:
            return 'there was an error adding your training to the db'

        return redirect(url_for('routes.home'))
    # if method is GET render homepage with above assigned values
    return render_template('home.html', user=current_user, curr_categories=curr_categories, user_trainings=user_trainings, current_week=curr_week, existing_weeks=existing_weeks)
 """
